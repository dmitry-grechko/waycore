...
Every Waycore app must include a `manifest.json` file in its root directory. This file declares the app's identity, capabilities, dependencies, and entry points.
...
This integrates with the OpenAPI spec at `/api/openapi.json` to auto-register AI tools.
